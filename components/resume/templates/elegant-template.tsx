...
export function ElegantTemplate({
  ...
  return (
    <div
      className="flex flex-col gap-6 p-10 font-serif text-slate-900"
      style={{ backgroundColor: PAGE_COLOR_HEX[pageColor] }}
    >
      <header className="flex flex-col items-center gap-2 text-center">
        {(content?.fullName || title) && (
          <h1 className="text-4xl font-medium tracking-[0.2em] uppercase">
            {content?.fullName || title}
          </h1>
        )}
        {contactLine && (
          <p className="text-sm tracking-wide opacity-80">{contactLine}</p>
        )}
